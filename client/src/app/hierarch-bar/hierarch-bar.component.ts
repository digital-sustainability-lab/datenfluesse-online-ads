...
export class HierarchBarComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.dataService.getCurrentDataSet().subscribe((data: any) => {
      ...
      this.root = d3
        .hierarchy(JSON.parse(JSON.stringify(data.hierarchy)))
  ...
  create_bar(svg: any, down: any, d: any, selector: any) {
    const g = svg
      ...
      .attr(
        'transform',
        `translate(0,${this.margin.top + this.barStep * this.barPadding})`
      )
  ...
  down(svg: any, d: any) {
    ...
    const enter = this.create_bar(svg, this.down, d, '.y-axis').attr(
      'fill-opacity',
      0
    );
    ...
    enter
      ...
      .attr(
        'transform',
        (d: any, i: any) => `translate(0,${this.barStep * i})`
      );
  ...
  up(svg: any, d: any) {
    ...
    // Update the x-axis.
    svg.selectAll('.x-axis').transition(transition1).call(this.xAxis);
    ...
    const enter = this.create_bar(svg, this.down, d.parent, '.exit').attr(
      'fill-opacity',
      0
    );
